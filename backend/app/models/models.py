...
class Company(Base, TimestampMixin):
    ...
    corporation_number: Mapped[str | None] = mapped_column(String(30), nullable=True)
...
class JournalEntry(Base, TimestampMixin):
    ...
    review_points: Mapped[str] = mapped_column(Text, default="")
...
class ClosingEntry(Base, TimestampMixin):
    ...
    amount: Mapped[float] = mapped_column(Float)
    description: Mapped[str] = mapped_column(String(300))
    lines = relationship("ClosingEntryLine", cascade="all, delete-orphan", back_populates="closing_entry")


class ClosingEntryLine(Base, TimestampMixin):
    __tablename__ = "closing_entry_lines"
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    closing_entry_id: Mapped[int] = mapped_column(ForeignKey("closing_entries.id"))
    account_id: Mapped[int] = mapped_column(ForeignKey("accounts.id"))
    debit: Mapped[float] = mapped_column(Float, default=0)
    credit: Mapped[float] = mapped_column(Float, default=0)
    closing_entry = relationship("ClosingEntry", back_populates="lines")
